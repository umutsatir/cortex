...
function DueBadge({ dueDate }: { dueDate: string }) {
  ...
  else                 { text = `Due in ${diff} days`;       color = 'var(--text-4)'; }
  return (
...
export function TaskItem({ task, dragIdPrefix = '', onRemove }: Props) {
  ...
  return (
    <>
      <motion.div
        ref={setNodeRef}
        style={{ ...dragStyle, backgroundColor: hovered ? 'var(--surface-2)' : 'transparent', transition: 'background-color 0.15s' }}
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between gap-2">
            <span
              className={`text-[13px] leading-snug flex-1 min-w-0 ${task.is_completed ? 'line-through' : ''}`}
              style={{ color: task.is_completed ? 'var(--text-4)' : 'var(--text-1)', cursor: 'pointer' }}
              onClick={() => selectTask(task.id)}
            >
              {task.title}
            </span>
            {task.estimated_minutes != null && task.estimated_minutes > 0 && !hovered && (
              <span className="text-[11px] flex-shrink-0 tabular-nums" style={{ color: 'var(--text-4)' }}>
            ...
            {hovered && (
              <button
                ...
                onPointerDown={(e) => e.stopPropagation()}
                onClick={(e) => { e.stopPropagation(); onRemove ? onRemove() : deleteTask(task.id); }}
          ...
          {hovered && (
            ...
            >
              <button
                ref={labelBtnRef}
                className="flex items-center gap-1.5 text-[11px] rounded-md px-1.5 py-0.5 transition-colors"
                style={{ color: label ? label.color : 'var(--text-4)' }}
              ...
              >
                <span
                  ...
                  style={{
                    ...
                    borderColor: label ? label.color : 'var(--border)',
                  }}
                />
              ...
              >
                <svg width="9" height="11" viewBox="0 0 9 11" fill="none">
                  <path
                    ...
                    stroke={task.priority ? PRIORITY_COLOR[task.priority] : 'var(--border)'}
                    strokeWidth="1.2"
